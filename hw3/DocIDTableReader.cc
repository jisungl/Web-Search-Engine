...
bool DocIDTableReader::LookupDocID(
  ...
  for (IndexFileOffset_t &curr : elements) {
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      Verify333(fseek(file_, curr + sizeof(DocIDElementHeader), SEEK_SET) == 0);
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file.
    IndexFileOffset_t bucketRecordOffset = offset_ + 
      sizeof(BucketListHeader) + i * sizeof(BucketRecord);
    Verify333(fseek(file_, bucketRecordOffset, SEEK_SET) == 0);
